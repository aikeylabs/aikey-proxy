...
// (Transport.Proxy == nil). Wrapping them in a SwappableClient — and rebuilding
// them on a network change — must NOT reintroduce env-proxy routing.
func TestSwappableDirect_BypassesEnvProxy(t *testing.T) {
	assertBypass := func(when string, c *http.Client) {
		...
		if tr.Proxy != nil {
			t.Fatalf("%s: Transport.Proxy != nil — control-plane client would route through the env proxy", when)
		}
	}
...
func TestRebuildAllControlPlane_RebuildsRegistered(t *testing.T) {
	...
	if a.Get().Transport.(*http.Transport).Proxy != nil {
		t.Fatal("rebuilt client reintroduced env-proxy routing")
	}
}
