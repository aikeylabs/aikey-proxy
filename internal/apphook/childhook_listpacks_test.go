...
func TestChildHook_ListPacks(t *testing.T) {
	...
	//
	// SpikeBaselinePreserved is FALSE for Wave6 through Wave9 on purpose: each
	// departs from the frozen Candidate-v9 stage set (Wave7 admits the two
	// anchored password-prefix rules at ordinal 3; Wave8 appends ordinal 13
	// required_positive_context_ceiling; Wave9 appends ordinal 14
	// hard_validator_confirmed) and carries its own measured safety delta
	// (SafetyDeltaVerified) instead. actionpolicy/bundle.go pins that per-wave;
	// asserting the value here proves it survives the IPC hop.
	//
	// Advanced wave-7 -> wave-9 on 2026-09-13 with the pointer. Wave-8 was built
	// and verified but never made active, so the bundle jumped two waves in one
	// step; only this literal moves, because the candidate/integration/history
	// SHAs below are wave-invariant by lineage.
	if rep.ActionPolicy.BundleSHA256 != "9992656d8842bb8330c713806f1008d9c6a2ce6395e84e3cf5e469ca9f639381" ||
