...
// SoleOrgID returns the one organization this vault's managed keys belong to.
//
// A cluster worker is provisioned for exactly ONE organization — its daemon
// pulls that org's delivery and nothing else — so the org id is derivable from
// what is already on disk. That is deliberately preferred over a new config
// field: a config field only takes effect after the installer is changed AND
// every node is redeployed, whereas this works on the fleet as it stands today.
//
// 🔴 It REFUSES rather than guesses. If the cache somehow holds keys from more
// than one org, "" is returned with ok=false and the caller must not proceed:
// picking one would point a node's policy — timeouts, cooldowns, the budget that
// governs whose requests get cut — at an organization chosen by row order. An
// empty vault likewise returns false; that is "not yet known", not "no org".
func (r *Reader) SoleOrgID() (string, bool) {
	if r == nil || r.db == nil {
		return "", false
	}
	rows, err := r.db.Query(`
		SELECT DISTINCT org_id FROM managed_virtual_keys_cache
		 WHERE org_id IS NOT NULL AND org_id <> '' LIMIT 2`)
	if err != nil {
		return "", false
	}
	defer rows.Close()
